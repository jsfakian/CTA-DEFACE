...
def find_defaced_nifti(nifti_out_dir: str, nifti_input_file: str) -> str:
    """
    Heuristically select the defaced CT NIfTI from CTA-DEFACE outputs.

    Strategy:
    - Base input: SeriesUID_0000 -> SeriesUID
    - In nifti_out_dir, consider all .nii* that do NOT contain "mask" in their name.
    - Prefer NIfTIs whose basename starts with SeriesUID.
    - If none match, but there is exactly 1 non-mask NIfTI, use that.
    ...
    candidates = []
    for f in sorted(glob.glob(os.path.join(nifti_out_dir, "*.nii*"))):
    ...
    if not candidates:
        # Fallback: if exactly one non-mask NIfTI, assume it's the defaced volume
        ...
        if len(non_mask) == 1:
            print(f"[defaced] Using single non-mask NIfTI: {non_mask[0]}")
            return non_mask[0]
        raise RuntimeError(
            "Could not identify defaced NIfTI for input "
            f"{nifti_input_file!r}. Found candidates: {non_mask}"
        )
    ...
    print(f"[defaced] Selected defaced NIfTI: {candidates[0]}")
    return candidates[0]
...
def prepare_dataset_for_write(ds: pydicom.dataset.Dataset):
    ...
    - Sets PixelData VR to OB (<=8 bits) or OW (>8 bits).
    - Forces implicit VR little endian transfer syntax if missing.
    """
    # Fix PixelData VR
    if "PixelData" in ds:
...
def nifti_to_dicom_fullref(nifti_file: str, ref_dicom_dir: str, output_dir: str):
    ...
    if candidates_dim0 and not candidates_dim2:
        ...
        # No perfect slice count match; choose closest series
    ...
    # Number of slices we can safely update
